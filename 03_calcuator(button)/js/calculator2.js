...
const elementDecimal = document.getElementById("decimal");
...
elementDecimal.addEventListener("click", function(){edit(".");});

/** 数字がクリックされたときの処理 */
function edit(wkInput) {
  if (wkInput === '.') {
    if (!elementResult.innerHTML.includes('.')) {
...
function keydownEvent(event){
  ...
  if(event.key === "+" || event.key === "-" || event.key === "*" || event.key === "/" || event.key === ".")update(event.key);
